...
// Sheet layout: D=PE(10), E=Exam(25), F=Conv(25)[formula], G=Practical, H=Viva, I=CT, J=Total[formula], K=Comment
const FORMAT_678 = {
  marks: [
    { col: "D", label: "PE (10)",                   max: 10 },
    { col: "E", label: "Exam (25)",                 max: 25 },
    { col: "G", label: "Practical Assignment (15)", max: 15 },
    { col: "H", label: "Practical Viva (5)",        max: 5  },
    { col: "I", label: "Practical CT (5)",          max: 5  },
  ],
  total: 50
};
const FORMAT_910 = {
  marks: [
    { col: "D", label: "PE (10)",                   max: 10 },
    { col: "E", label: "Exam (25)",                 max: 25 },
    { col: "G", label: "Practical Exam (15)",       max: 15 },
    { col: "H", label: "Practical Viva (5)",        max: 5  },
    { col: "I", label: "Practical Notebook (5)",    max: 5  },
  ],
  total: 50
};
...
function buildStudentCard(s, classKey) {
  ...
  // PE + Exam দেখাবে, তারপর Conv(25) calculated row, তারপর practicals
  const pe   = Number(getMarkValue(s, "PE (10)"))   || 0;
  const exam = Number(getMarkValue(s, "Exam (25)")) || 0;
  const hasExamData = (getMarkValue(s, "PE (10)") !== "" || getMarkValue(s, "Exam (25)") !== "");
  const conv = hasExamData ? Math.round((pe + exam) / 35 * 25 * 100) / 100 : "";
  ...
  const rows = fmt.marks.map(m => {
    ...
    // PE এর পরে Conv(25) row insert করব
    const convRow = (m.col === "E") ? `
      <tr style="background:#f0f4ff;">
        <td class="sc-label" style="font-style:italic;color:#666;">↳ Conv (25) <span style="font-size:7.5pt;color:#aaa;">(PE+Exam)/35×25</span></td>
        <td class="sc-value">${conv !== "" ? `<span class="sc-mark" style="color:#1a3a6b;">${conv}</span><span class="sc-max"> / 25</span>` : `<span class="sc-pending">Not yet conducted</span>`}</td>
      </tr>` : "";
    return `
      <tr>
...
async function saveEdit() {
  ...
  if (!valid) return;
  colValues["K"] = document.getElementById("ef_J").value;
  const s = editTarget.student;
  fmt.marks.forEach(m => {
    s[m.label] = colValues[m.col] === "" ? "" : colValues[m.col];
  });
  s["Comment"] = colValues["K"];
  // Total = Conv(25) + Practical cols — Conv = (PE+Exam)/35*25
  const pe   = Number(colValues["D"]) || 0;
  const exam = Number(colValues["E"]) || 0;
  const conv = Math.round((pe + exam) / 35 * 25 * 100) / 100;
  const practicalTotal = ["G","H","I"].reduce((sum, col) => sum + (Number(colValues[col]) || 0), 0);
  s["Total"] = Math.round((conv + practicalTotal) * 100) / 100;
